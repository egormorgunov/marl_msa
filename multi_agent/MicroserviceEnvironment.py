...
class RoadEnvironment:
    ...
    def reset(self):
        ...
        self.total_reward = 0
        print(f"Environment for agent {self.agent_id} reset.")
    ...
    def step(self, action):
        ...
        if not self.is_done():
            if self.current_stage == 0:
                clear_table_info(f"public.booking{self.agent_id}_info")
                if action == 0 or action == 1:
                    self.current_stage = 1
                    print(f"Agent {self.agent_id} made 1 step")
                elif action == 2:
                    print('Service waits')
                ...
                if action == 0:
                    ...
                    if self.weather_id == "Sun":
                        ...
                        self.weather_id = 1
                    print(f"Agent {self.agent_id} COEFFICIENTS = ", self.weather_id, self.car_coef, self.train_coef, self.plane_coef)
                    print(f"Agent {self.agent_id} made 2 step to collect weather")
                elif action == 1:
                    self.current_stage = 0
                    print(f"Agent {self.agent_id} made 2 step to return")
                elif action == 2:
                    print('Service waits')
                ...
                if action == 0:
                    ...
                    print(f"Agent {self.agent_id} made 3 step, road {action + 1} selected")
                elif action == 1:
                ...
                if action == 0:
                    ...
                    reward = -3
                    print(f'Agent {self.agent_id} skipped booking')
                elif action == 1:
                    self.current_stage = 6
                    self.status = 'Active'
                    self.action4_counter += 1
                    reward = -1
                    insert_booking_info(self.transport, self.status, f"public.booking{self.agent_id}_info")
                    print(f'Agent {self.agent_id} booking successful')
                elif action == 2:
                    self.current_stage = 6
                    self.status = 'Inactive'
                    reward = -2
                    insert_booking_info(self.transport, self.status, f"public.booking{self.agent_id}_info")
                    print(f'Agent {self.agent_id} booking incorrect')
                ...
                if action == 0:
                    ...
                    print(f"Agent {self.agent_id} made 4 step")
                elif action == 1:
                    self.current_stage = 5
                    # clear_table_info(f"public.booking{self.agent_id}_info")
                    print(f"Agent {self.agent_id} made 4 step to return")
                elif action == 2:
                    print('Service waits')
            elif self.current_stage == 7:
                if action == 0 or action == 1 or action == 2:
                    print(f"Agent {self.agent_id} made 5 step")
                    self.is_done()
                    return True
        ...
        state = [self.current_stage, action, self.weather_id]
        print(f"Agent {self.agent_id} State = ", state)
        return reward, state, self.is_done(), self.action1_counter, self.action2_counter, self.action3_counter, \
            self.action4_counter
    ...
    def print_current_stage(self):
        ...
        print(f"Agent {self.agent_id} Current stage: {stage_info['name']}")
